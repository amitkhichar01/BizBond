...
const SkillsSection = ({ user, editBtn }) => {
    const [editForm, setEditForm] = useState(false);

    return (
        <section className="bg-white  rounded-lg mt-5 p-5 tracking-wide relative">
            {editForm && <EditForm number="1" heading="About" names={["Skills"]} values={[user.skills]} userId={user._id.toString()} />}
            {editBtn && (
            ...
            {user.skills.length ? (
                ...
                <p>Showcase your skills to highlight what you excel at.</p>
            )}
        </section>
    );
};
