...
export default function Posts() {
  ...
  const updatePostLikes = (postId, likes) => {
    setPosts((prevPosts) =>
      prevPosts.map((post) => (post._id === postId ? { ...post, likes } : post))
    );
  };
  ...
  return (
    ...
    {isLoading && <Loader/>}
      ...
      <div className="flex flex-col items-start h-screen font-nunito pt-16">
        <div className="sm:py-7 flex flex-col gap-7 justify-center items-center w-full">
          {posts ? (
            posts.map((post) => (
              <section
                key={post._id}
                className="bg-white text-black w-full sm:w-[50vw] rounded"
              >
                <div className="m-4">
                  <div className="flex items-start mb-4">
                    <Link href={`/user/${post.author._id}`}>
                      <Image
                        src={applyImageTransformation(
                          post.author.profile_url,
                          "w_400,h_400,c_thumb,g_face"
                        )}
                        width={100}
                        height={100}
                  ...
                  <div className="min-h-20">
                    <p className="mb-5 text-md sm:text-lg indent-4">
                      {post.content}
                    </p>
                    {post.image_url && (
                      <Image
                        ...
                        height={1080}
                        alt="Post image"
                        className="object-fill w-full h-[60vh] rounded-md"
                        priority
                      />
                    )}
                  </div>
                          ...
                          <>
                            ...
                            <form
                              onSubmit={handleSubmit((data) =>
                                handleCommentSubmit(data, post._id)
                              )}
                            ...
                            >
                              ...
                              <div className="self-end">
                                <button
                                  type="submit"
                                  className="bg-blue-600 text-white px-3 sm:px-4 py-2 text-sm sm:text-md rounded self-end"
                                >
                                  Comment
                                </button>
                              </div>
                          ...
                          <div key={index} className="mt-10">
                            <div className="flex justify-start items-start gap-4">
                              ...
                              <div className="flex flex-col items-start justify-center">
                                ...
                                <p className="hyphens-manual text-lg indent-1">
                                  {comment.content}
                                </p>
                              </div>
                            </div>
        ...
        {sharePost && (
          <SharePopup post={sharePost} setSharePost={setSharePost} />
        )}
      </div>
    </>
  );
